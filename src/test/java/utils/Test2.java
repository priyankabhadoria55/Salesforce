...
//            System.out.println(FinalTesttoExecute);
    ...
    public static void modifyFile1(String filePath, String ScenerioName, String oldString, String newString) {
        ...
        try {
            ...
            while (line != null) {
                if(line.contains("@Test")&&line.contains(ScenerioName)&& !line.contains("@TestInfo"))
                {
...
//                            System.out.println(line);
    ...
    public static void GettingFailedTestDetails(ITestResult iTestResult)
    {
        String FailedCase = null;
        try {
            FailedCase= readproperty("FailedTestScenerio");
        } catch (IOException e) {
            e.printStackTrace();
        }
        if(FailedCase.trim().equalsIgnoreCase("NA"))
        {
            FailedCase="";
        }
        FailedCase= FailedCase+","+getTestDescription(iTestResult);
        try {
            updateproprtywithvalue("FailedTestScenerio",FailedCase);
        } catch (IOException e) {
            e.printStackTrace();
        }
}

    public static void ResetFailedTestDetailes()
{
    try {
        updateproprtywithvalue("FailedTestScenerio","NA");
    } catch (IOException e) {
        e.printStackTrace();
    }
}
public static void EnableLastFailedTestInCurrentExecution(String filePath)
{
    ArrayList<String> FailedScenario = new ArrayList<>();
    String FailedCase = null;
    try {
        FailedCase= readproperty("FailedTestScenerio");
    } catch (IOException e) {
    ...
    if(!FailedCase.trim().equalsIgnoreCase("NA")) {
        String[] args = FailedCase.split(",");
        for (int k = 1; k < args.length; k++) {
          String Scenerio=  modifyFile3(filePath, args[k], "false", "true");
            FailedScenario.add(Scenerio);
        }
    }
    try {
        updateproprtywithvalue("ResetFailedSceneriotoFalseAfterExecution",String.join(", ", FailedScenario).trim().substring(1,String.join(", ", FailedScenario).length()-2));
    } catch (IOException e) {
        e.printStackTrace();
    }

}
    public static void RunFailedTestinCurrentExecution() {

        try {
            if(readproperty("NeedtoRunFailedTest").equalsIgnoreCase("Yes"))
            {
                EnableLastFailedTestInCurrentExecution( ".\\src\\test\\java\\testScript\\Sample.java");
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        public static void ResetFailedTesttoFalse(String filePath)
    {
        String FailedCase = null;
        try {
            FailedCase= readproperty("ResetFailedSceneriotoFalseAfterExecution");
        } catch (IOException e) {
            e.printStackTrace();
        }
        if(!FailedCase.contains("NA")) {
            String[] args = FailedCase.split(",");
            for (int k = 0; k < args.length; k++) {
                modifyFile1(filePath, args[k], "true", "false");
            }
        }
    }
    public static void ResetFailedTestDetailes2()
    {
        try {
            updateproprtywithvalue("ResetFailedSceneriotoFalseAfterExecution","NA");
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    public static void ResetFailedTestExecued() {
        ResetFailedTesttoFalse(".\\src\\test\\java\\testScript\\Sample.java");
        ResetFailedTestDetailes2();
    }

    public static String modifyFile3(String filePath, String ScenerioName, String oldString, String newString) {
        File fileToBeModified = new File(filePath);
       String FailedScenerioName="";
        String oldContent = "";
        ...
        try {
            ...
            while (line != null) {
                if(line.contains("@Test")&&line.contains(ScenerioName)&& !line.contains("@TestInfo")&&line.contains("false"))
                {
                    oldContent= oldContent + line.replace(oldString, newString) + System.lineSeparator();
                    FailedScenerioName=FailedScenerioName+","+ScenerioName;
                }
                else {
                    oldContent = oldContent + line + System.lineSeparator();
                }
